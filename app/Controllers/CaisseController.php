...
// ============================================
    public function statistiques()
    {
        ...
        $totalAchats = $achatModel->where('statut', 'cloture')->countAllResults();

        // Chiffre d'affaires total
        $caTotal = $achatModel->select('SUM(total) as total')
            ->where('statut', 'cloture')
            ->first();
        $totalCA = $caTotal ? $caTotal->total : 0;
        ...
        $achatsParUser = $achatModel->select('utilisateurs.nom_complet, COUNT(achats.id_achat) as nb_achats, SUM(achats.total) as total')
            ->join('utilisateurs', 'utilisateurs.id_user = achats.id_user', 'left')
            ->where('achats.statut', 'cloture')
            ->groupBy('achats.id_user')
            ->orderBy('nb_achats', 'DESC')
            ->findAll();

        // Achats par jour (7 derniers jours)
        $achatsParJour = [];
        for ($i = 6; $i >= 0; $i--) {
            $date = date('Y-m-d', strtotime("-$i days"));
            $count = $achatModel->where('statut', 'cloture')
        ...
        $data = [
            ...
            'achatsParUser' => $achatsParUser,
            'achatsParJour' => $achatsParJour
        ];
